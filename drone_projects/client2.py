...
class UavThread(threading.Thread):
    ...
    def run(self):
        ...
        databuffer =b''
        while True: 
            data, _ = self.sock.recvfrom(1)      # buffer size is 4096 bytes
            # print('from '+str(addr))
...
#保存文件数据
            ...
            #     continue
            data, _ = self.sock.recvfrom(1)      # buffer size is 4096 bytes
            len2 = struct.unpack("B", data[0])
            len = int.from_bytes(len2, byteorder='little')-3
            print(" len "+len)
            while(len(databuffer)< len):
                data, _ = self.sock.recvfrom(len-len(databuffer))      # buffer size is 4096 bytes
                databuffer+=data
